...
func TestRCAService_InitializationWithConfig(t *testing.T) {
	...
	// 1. 设置自定义配置
	customConfig := services.AutoRCAConfig{
		Enabled:   true,
		RateLimit: 20,
		Period:    120, // 20次/120秒 = 0.1667次/秒
	}
	_, err := settingService.SetSetting(services.SettingKeyAutoRCA, customConfig, "Custom Rate Limit")
...
func TestRCAService_DynamicConfigUpdate(t *testing.T) {
	...
	// 1. 初始配置
	initialConfig := services.AutoRCAConfig{
		Enabled:   true,
		RateLimit: 10,
		Period:    60,
	}
	_, err := settingService.SetSetting(services.SettingKeyAutoRCA, initialConfig, "Initial")
	assert.NoError(t, err)
	rcaService := NewRCAService(database, nil, nil, settingService, nil, nil)
	...
	// 3. 更新配置
	newConfig := services.AutoRCAConfig{
		Enabled:   true,
		RateLimit: 30,
		Period:    60,
	}
	_, err = settingService.SetSetting(services.SettingKeyAutoRCA, newConfig, "Updated")
	assert.NoError(t, err)
