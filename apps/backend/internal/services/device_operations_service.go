...
func (s *DeviceOperationsService) DrainNodes(ctx context.Context, ciCodes []string, opts DrainOptions) (*BatchOperationResult, error) {
	...
	for _, d := range devices {
		...
		go func(device navy.Device) {
			...
			// 调用 SafeDrainService
			resp, err := s.safeDrainService.StartDrain(ctx, &SafeDrainRequest{
...
func (s *DeviceOperationsService) ShutdownNodes(ctx context.Context, ciCodes []string) (*JobHandle, error) {
	return s.executePowerOperation(ctx, ciCodes, "shutdown")
}

// RebootNodes 重启节点 (通过 AWX)
func (s *DeviceOperationsService) RebootNodes(ctx context.Context, ciCodes []string) (*JobHandle, error) {
	return s.executePowerOperation(ctx, ciCodes, "reboot")
}

// executePowerOperation 执行电源操作
func (s *DeviceOperationsService) executePowerOperation(ctx context.Context, ciCodes []string, operation string) (*JobHandle, error) {
	if s.awxRuntime == nil {
	...
	// 启动 AWX Job
	jobHandle, err := s.awxRuntime.LaunchJob(ctx, JobConfig{
