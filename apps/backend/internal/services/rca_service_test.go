...
func TestRCAService_TriggerAndRateLimit(t *testing.T) {
	...
	_, err := settingService.SetSetting(SettingKeyAutoRCA, config, "Strict Limit")
...
func TestRCAService_QueueProcessing(t *testing.T) {
	...
	// 1. Enable Auto-RCA
	config := AutoRCAConfig{Enabled: true, RateLimit: 10, Period: 60}
	_, err := settingService.SetSetting(SettingKeyAutoRCA, config, "")
	assert.NoError(t, err)
