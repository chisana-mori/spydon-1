...
func (s *RCAService) fetchAutoRCAConfig() (*AutoRCAConfig, time.Time, error) {
...
func (s *RCAService) loadAutoRCAConfig() *AutoRCAConfig {
...
func (s *RCAService) isSeverityAllowed(config *AutoRCAConfig, severity string) bool {
	if len(config.AllowedSeverities) == 0 {
		return true
	}
...
func (s *RCAService) checkRateLimit(config *AutoRCAConfig) (bool, error) {
	if config == nil {
		return true, nil
	}
...
func (s *RCAService) updateRateLimiter(config *AutoRCAConfig) {
...
func (s *RCAService) triggerRCA(alert *models.Alert, config *AutoRCAConfig, enforceSeverity bool) (*models.RCARun, error) {
	if !config.Enabled {
