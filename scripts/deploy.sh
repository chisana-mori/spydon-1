...
build_images() {
    ...
    # 构建后端镜像
    log_info "构建后端镜像..."
    docker build -t robusta-central-hub/backend:latest ./backend
    
    # 构建前端镜像
    log_info "构建前端镜像..."
    docker build -t robusta-central-hub/frontend:latest ./frontend
...
deploy_docker() {
    ...
    if [[ "$with_holmes" == "true" ]]; then
        compose_cmd="$compose_cmd --profile holmes"
    fi
