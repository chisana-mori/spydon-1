...
export default function Alerts() {
  ...
  const { data: alertsData, isLoading } = useQuery({
    queryKey: ['alerts-timeline', filters, searchTerm, timeRange],
  ...
  const timelineAlerts = useMemo(() => {
    return alerts
      .filter(alert => {
        ...
        return alertTime >= timeRangeStart && alertTime <= timeRangeEnd
      })
      .sort((a, b) => {
  ...
  }, [alerts, timeRangeStart, timeRangeEnd])
      ...
      <div className="bg-white border rounded-lg p-4">
        ...
        <div className="flex items-center gap-6">
          ...
          <div className="flex items-center gap-2">
            ...
            <div className="flex gap-1">
              <Button
                variant={!filters.cluster_id ? "default" : "ghost"}
                size="sm"
                onClick={() => handleFilterChange('cluster_id', '')}
              ...
              {clusters.slice(0, 3).map((cluster) => (
                <Button
                  ...
                  variant={filters.cluster_id === cluster.cluster_id ? "default" : "ghost"}
                  size="sm"
                  onClick={() => handleFilterChange('cluster_id', cluster.cluster_id)}
                  className={`h-7 px-3 text-xs rounded-full ${
                    filters.cluster_id === cluster.cluster_id
                      ? 'bg-black text-white hover:bg-gray-900'
                      : 'bg-gray-100 text-gray-700 hover:bg-gray-200'
                  }`}
                >
