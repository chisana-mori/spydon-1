...
func TestClusterHeartbeat(t *testing.T) {
	handler, router := setupTestHandler()
	router.POST("/clusters/heartbeat", handler.ClusterHeartbeat)

	payload := ClusterHeartbeatRequest{
	...
	req, err := http.NewRequest("POST", "/clusters/heartbeat", bytes.NewBuffer(jsonPayload))
	require.NoError(t, err)
	req.Header.Set("Content-Type", "application/json")
	...
	err = json.Unmarshal(w.Body.Bytes(), &response)
	require.NoError(t, err)
	assert.Equal(t, "心跳更新成功", response["message"])
}
